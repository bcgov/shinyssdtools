...
app_server <- function(input, output, session) {
  ...
  fit_dist <- reactive({
    ...
    dist <- input$selectDist
    x <-  try(ssdtools::ssd_fit_dists(data, left = conc,
  ...
  plot_model_average <- reactive({
    ...
    plot_predictions(data, pred, conc = conc, label = label, colour = colour, 
  ...
  output$describeCl <- renderText({
    ...
    HTML(desc1, tr("ui_3cldesc2", trans()), 
  ...
  output$dlFitRds <- downloadHandler(
    filename = function() {"ssdtools_distFitPlot.rds"},
    content = function(file) {
  ...
  output$dlPredRds <- downloadHandler(
    filename = function() {"ssdtools_modelAveragePlot.rds"},
    content = function(file) {
  ...
  output$codePredCl<- renderUI({
    ...
    conf <- paste0(paste0(form, "(dist, ", arg, " = "), thresh, "L, ci = TRUE",
  ...
  output$ui_2dlplot <- renderUI({
    downloadButton("dlFitPlot", label = tr("ui_2dlplot", trans()),
                   style = 'padding:4px; font-size:80%')
  })
  
  output$ui_2dlrds <- renderUI({
    downloadButton("dlFitRds", label = tr("ui_2dlrds", trans()),
                   style = 'padding:4px; font-size:80%')
  })
  
  output$ui_2dltable <- renderUI({
    downloadButton("dlFitTable", label = tr("ui_2dltable", trans()),
                   style = 'padding:4px; font-size:80%')
  })
  ...
  output$ui_3thresh <- renderUI({
    req(input$thresh_type)
    if(input$thresh_type == "Concentration"){
      return(numericInput("conc", label = tr("ui_3conc", trans()), 
                     value = 1, min = 0,
                     max = 100, step = 0.1, width = "100px"))
    }
    numericInput("thresh", label = tr("ui_3thresh", trans()),
                 value = 5, min = 0,
  ...
  output$ui_3samples <- renderUI({
    ...
    selectInput('bootSamp', label = tr("ui_3samples", trans()),
                choices = choices,
                selected = choices[2],
                width = "150px")
  })
  
  thresh_rv <- reactiveValues(percent = NULL,
                              conc = NULL)
  ...
  output$ui_3dlplot <- renderUI({
    downloadButton("dlPredPlot", label = tr("ui_2dlplot", trans()),
                   style = 'padding:4px; font-size:80%')
  })
  
  output$ui_3dlrds <- renderUI({
    downloadButton("dlPredRds", label = tr("ui_2dlrds", trans()),
                   style = 'padding:4px; font-size:80%')
  })
  
  output$ui_3dltable <- renderUI({
    downloadButton("dlPredTable", label = tr("ui_2dltable", trans()),
                   style = 'padding:4px; font-size:80%')
  })
