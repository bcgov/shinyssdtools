...
ui <- function() {
  tagList(
    ...
    shinyjs::useShinyjs(),
    waiter::use_waiter(),
    tagList(tags$link(rel = "stylesheet", type = "text/css", href = "style.css")),
    tags$style(
      type = "text/css",
...
server <- function(input, output, session) {
  ...
  output$ui_about <- renderUI({
    ...
    if (translation.value$lang == "English"){
      return({
        tagList(
          p(ver),
          includeMarkdown("extdata/about_en.md")
        )
      })
    } else {
      return({
        tagList(
          p(ver),
          includeMarkdown("extdata/about_fr.md")
        )
      })
    }
  })
  ...
  output$ui_userguide <- renderUI({
    if (translation.value$lang == "English") {
      return(includeMarkdown("extdata/user_en.md"))
    }
    includeMarkdown("extdata/user_fr.md")
  })
}
